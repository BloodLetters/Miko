...
const SearchPage = ({ onMangaSelect }) => {
  ...
  const [searchSource, setSearchSource] = useState(initialSource);
  const [comicType, setComicType] = useState(initialType);
  ...
  const [lastSearchQuery, setLastSearchQuery] = useState(lastQuery);
  
  // Temporary states untuk menyimpan perubahan filter sebelum search
  const [tempSource, setTempSource] = useState(searchSource);
  const [tempType, setTempType] = useState(comicType);

  // Hanya simpan hasil pencarian ke localStorage
  useEffect(() => {
  ...
  const handleSearch = async(e) => {
    ...
    // Update dan simpan filter saat melakukan pencarian
    ...
    try {
      ...
      if (data.status === 200) {
        ...
        setSearchResults(prev => 
          isNewSearch ? data.data.results : [...prev, ...data.data.results]
        );
